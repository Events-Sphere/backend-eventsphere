...
export const createBooking = async (req: Request | any, res: Response) => {
  try {
    ...
    return ApiResponseHandler.success(res, bookingResponse.data, COMMON_MESSAGES.BOOKING_INITIATED, 201);
  } catch (error: any) {
...
export const confirmBooking = async (req: Request | any, res: Response) => {
  try {
    ...
    const { event_id: eventId, payment_method: paymentMethod, payment_id: paymentId, sub_event_items: subEventItems } = req.body.data;
