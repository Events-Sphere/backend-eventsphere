...
export const addToFavorite = async (req: Request, res: Response) => {
  try {
    ...
    if (!response.status) {
      return ApiResponseHandler.error(
        res,
        `${response.message ?? "Failed to add favorite event."}`,
        400
      );
    }
...
export const removeFromFavorite = async (req: Request, res: Response) => {
  try {
    ...
    if (!response.status) {
      return ApiResponseHandler.error(
        res,
        `${response.message ?? "Failed to remove favorite event."}`,
        400
      );
    }
...
export const getFavoriteEvents = async (req: Request, res: Response) => {
  try {
    ...
    const response = await userInstance.getFavoriteEventList(
      Number(req.user.id)
    );
    ...
    if (!response.status) {
      return ApiResponseHandler.error(
        res,
        `${response.message ?? "No favorite events found."}`,
        404
      );
    }
